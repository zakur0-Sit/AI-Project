...
def create_constraints_app(professors):
    ...
    def display_constraint(professor, constraint):
        ...
        label = tk.Label(frame, text=f"{professor}: {constraint}", anchor="w", bg="#F0F8FF", font=("Arial", 12))
    ...
    def display_student_constraint(student_constraint):
        ...
        label = tk.Label(frame, text=f"Student: {student_constraint}", anchor="w", bg="#FFFACD", font=("Arial", 12))
        label.pack(side="left", fill="x", expand=True, padx=5)
    ...
    def close_window():
        constraints_data = {
            "professor_constraints": constraints_list,
            "student_constraints": student_constraints_list
        }
    ...
    ttk.Label(prof_frame, text="Select Professor:", font=("Arial", 12)).pack(pady=5)
    professor_var = tk.StringVar()
    professor_menu = ttk.Combobox(prof_frame, textvariable=professor_var, values=professors, state="readonly", width=30, font=("Arial", 12))
    professor_menu.pack(pady=5)

    # Textbox pentru adăugarea constrângerii
    ttk.Label(constraint_frame, text="Professor's constraint:", font=("Arial", 12)).pack(pady=5)
    constraint_entry = ttk.Entry(constraint_frame, width=50, font=("Arial", 12))
    constraint_entry.pack(pady=5)

    # Buton pentru a adăuga constrângerea
    add_button = tk.Button(top_frame, text="Add Professor Constraint", command=add_constraint, bg="#98FB98", bd=0, width=40, height=2, font=("Arial", 12))
    ...
    ttk.Label(student_constraints_frame, text="Student's constraint:", font=("Arial", 12)).pack(pady=5)
    student_constraint_entry = ttk.Entry(student_constraints_frame, width=50, font=("Arial", 12))
    student_constraint_entry.pack(pady=5)

    add_student_button = tk.Button(top_frame, text="Add Student Constraint", command=add_student_constraint, bg="#98FB98", bd=0, width=40, height=2, font=("Arial", 12))
    ...
    tk.Label(result_professors_frame, text="Professors Constraints:", font=("Arial", 12)).pack(pady=5)
    constraints_frame = tk.Frame(result_professors_frame, bg="#F0F8FF")
    constraints_frame.pack(fill="both", pady=5, padx=5)

    tk.Label(result_students_frame, text="Students Constraints:", font=("Arial", 12)).pack(pady=5)
    ...
    save_button = tk.Button(bottom_frame, text="Save and Close", command=close_window, bg="#ADD8E6", bd=0, width=40, height=2, font=("Arial", 12))
